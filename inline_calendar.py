...
def init(chat_id, base_date, min_date, max_date,
    ...
    m_names = ['-']
    if not month_names:
        m_names.extend(['Январь', 'Февраль', 'Март', 'Апрель', 'Мая', 'Июнь', 'Июль', 'Август', 'Сентябрь', 'Октябрь', 'Ноябрь', 'Декабрь'])
    else:
    ...
    if len(days_names) != 7:
        raise Exception('Нет такого дня недели')
...
def handler_callback(chat_id, callback):
    ...
    if not _check_callback(callback):
        raise WrongCallbackException('Ошибка вызова')
